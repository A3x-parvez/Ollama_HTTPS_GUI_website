...
st.set_page_config(page_title="Ollama-Chat-v5", page_icon="Chatbot.ico", layout="wide")
...
def save_chat_history(user_message, ai_message, model):
    ...
    with open(filename, "w", encoding="utf-8") as file:
        file.write(f"<span style='font-size:19px; color: #ff5c33; font-weight: bold;'>👤You :</span><br>{user_message}<br><span style='font-size:19px; color: #ff5c33; font-weight: bold;'>🤖AI :</span><br>Generated By <span style='color: green; font-weight: bold;'>{model}</span><br>{ai_message}")
...
with st.sidebar:
    ...
    new_model = st.text_input('Your label', placeholder="Enter model name to install:", label_visibility="collapsed")
    ...
    if selected_chat:
        ...
        st.markdown(
            ...
            <html>
            <body>
            <div style="max-height: 400px; overflow-y: auto; padding: 10px; border: 2px solid #ddd; border-radius: 10px; background-color: #000000;">
            {chat_content}
            </div>
            </body>
            </html>
    ...
    if st.button("Clear This Chat", use_container_width=True):
        if selected_chat:  # Ensure a file is selected
            st.session_state.confirm_delete = True  # Activate confirmation state
            # st.rerun()
        else:
            st.warning("⚠️ No chat selected!")

    # Confirmation step (only appears after clicking "Clear This Chat")
    if st.session_state.confirm_delete:
        st.warning(f"⚠️ Are you sure you want to delete '{selected_chat}'?")
        
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Yes", use_container_width=True):
                os.remove(os.path.join(CHAT_DIR, selected_chat))
                st.toast(f"✅ Deleted: {selected_chat}", icon="✅")
...
prompt = st.text_area("Enter text :", placeholder="Type your message here...", label_visibility="collapsed")
# cf2804
# Inject custom CSS to change button color
st.markdown("""
    <style>
    .stButton>button {
        background-color: #d82f0a;  /* Replace with your desired color */
        color: white;
        border: none;
        padding: 10px 20px;
        border-radius: 5px;
...
if st.button("Generate Response",type="secondary"):
    if prompt.strip():
        with st.spinner("🤔 Thinking..."):
            try:
                ...
                save_chat_history(prompt, bot_response,OLLAMA_MODEL)
